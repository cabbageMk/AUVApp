...
import com.zzz.common.ext.sharedpreferences.getSpValue
import com.zzz.common.ext.sharedpreferences.putSpValue
import java.util.*
...
object GlobalUtil {
    ...
    val appPackage: String
        get() = BaseApp.context.packageName
     ...
     */
    val appName: String
        get() = BaseApp.context.resources.getString(BaseApp.context.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = BaseApp.context.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            BaseApp.context.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            BaseApp.context.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(BaseApp.context.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
            ...
            var uuid = BaseApp.context.getSpValue("uuid", "")
            if (!TextUtils.isEmpty(uuid)) {
                deviceSerial = uuid
                return deviceSerial.toString()
            }
            uuid = UUID.randomUUID().toString().replace("-", "").toUpperCase(Locale.getDefault())
            BaseApp.context.putSpValue("uuid", uuid)
            deviceSerial = uuid
            return deviceSerial.toString()
        } else {
            return deviceSerial.toString()
    ...
    fun getString(resId: Int): String {
        return BaseApp.context.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return BaseApp.context.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return BaseApp.context.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = BaseApp.context.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            BaseApp.context.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = BaseApp.context.packageManager
